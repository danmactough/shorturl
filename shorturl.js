...
var env = process.env.NODE_ENV = process.env.NODE_ENV || config.env || 'development';

var red = express();

red.set('db-uri', models.dbUri[env]);
...
main.set('db-uri', models.dbUri[env]);
...
main.use(session({
  store:             new MongoStore({ url: main.set('db-uri') }),
...
if (!module.parent) {
  http.createServer(red).listen(config.port_redirector, config.host_redirector || '0.0.0.0', function (err) {
    debug("%s v%s listening on port %d in %s mode", red.get('package.name'), red.get('package.version'), this.address().port, red.settings.env);
  });

  http.createServer(main).listen(config.port_main, config.host_main || '0.0.0.0', function (err) {
    debug("%s v%s listening on port %d in %s mode", main.get('package.name'), main.get('package.version'), this.address().port, main.settings.env);
  });
}
